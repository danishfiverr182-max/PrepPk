...
export default function TakeTestPage() {
  ...
  // Fit the question/options block to the available space instead of
  // letting it scroll — shrinks just enough for long questions/options to
  // still appear fully on screen. Recalculates on every question change.
  const { containerRef, contentRef, scale, needsScrollFallback } =
    useFitToContainer([currentIndex, currentMcq?.question, currentMcq?.imageUrl]);
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      {/* ── App Top Header ────────────────────────────────────── */}
      <header className="min-h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-3 sm:px-6 py-2 shrink-0 flex items-center justify-between gap-2 sm:gap-3">
        <div className="flex items-center gap-1.5 sm:gap-3 min-w-0 shrink">
          <Link
            ...
            className="flex items-center gap-1.5 text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 text-sm font-semibold transition shrink-0"
          >
            <svg
              className="w-4 h-4"
              fill="none"
          ...
          <h2 className="font-heading font-bold text-slate-800 dark:text-white text-xs sm:text-sm md:text-base truncate min-w-0">
            {sectionName}
          </h2>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          <button
            ...
            className="lg:hidden bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-750 dark:text-slate-300 px-2.5 sm:px-3.5 py-2 rounded-xl text-[11px] sm:text-xs font-bold transition flex items-center gap-1.5"
          >
            <span className="hidden sm:inline">Grid </span>({answeredCount}/{total})
          </button>
          ...
          <button
            ...
            disabled={submitting}
            className="bg-brand hover:bg-brand-dark disabled:opacity-70 disabled:hover:scale-100 text-white px-3 sm:px-4 py-2 rounded-xl text-[11px] sm:text-xs font-bold shadow-sm transition hover:scale-105 flex items-center gap-1.5"
          >
            {submitting && (
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* Main Content Area — the question/options block is shrunk to fit
            this space (see useFitToContainer) instead of scrolling, with a
            fixed-position Prev/Next/Submit bar pinned below it (outside
            this area) so those buttons stay in the same spot on screen
            regardless of how long the question text or options are. */}
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          <main
            className={`flex-1 px-3 sm:px-6 py-4 sm:py-6 flex flex-col items-center ${
              needsScrollFallback ? "overflow-y-auto" : "overflow-hidden"
            }`}
          >
            <div
              ref={containerRef}
              className={`w-full ${currentMcq?.imageUrl ? "max-w-6xl" : "max-w-5xl"} flex-1 flex flex-col`}
            >
              {/* Question Details — scaled to fit containerRef's available
                  height. scrollHeight (used to compute the scale) reflects
                  this block's natural size regardless of the transform
                  already applied, so this stays accurate question to
                  question without needing to reset scale first. */}
              <div
                ref={contentRef}
                className="w-full"
              ...
              >
                ...
                <div className="space-y-6 mcq-no-copy" {...noCopyProps}>
                  ...
                  {currentMcq.imageUrl ? (
                    // On mobile this stacks vertically and relies on the outer
                    // <main> for scrolling (single scroll surface). On lg+ it
                    // becomes a fixed-height split pane where the right panel
                    // scrolls independently — that only gets `overflow-y-auto`
                    // from `lg:` up, so phones never end up with two nested
                    // scroll containers fighting each other (the classic
                    // "scroll gets stuck" mobile bug).
                    <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 lg:h-[74vh] h-auto min-h-0">
                      {/* Large Image Section */}
                      <div className="lg:w-[55%] w-full bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-2 sm:p-3 lg:p-4 shadow-lg overflow-hidden flex flex-col min-h-0 lg:h-full h-[32vh] sm:h-[38vh]">
                      ...
                      <div className="lg:w-[47%] w-full min-w-0 flex-shrink-0 lg:overflow-y-auto min-h-0">
                        <div className="bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-7 shadow-sm h-fit">
                          {/* Question Metadata */}
          ...
          {/* ── Fixed Prev / Next / Submit bar ─────────────────────
              Lives outside the scrollable <main>, so its position on
              screen never shifts as question/option text runs 1, 2, or
              3 lines — only the content above it scrolls. */}
          {currentMcq && (
            <div className="shrink-0 border-t border-slate-200 dark:border-slate-850 bg-white dark:bg-slate-900 px-3 sm:px-6 py-3 sm:py-4">
              <div
                className={`w-full ${currentMcq?.imageUrl ? "max-w-6xl" : "max-w-5xl"} mx-auto flex items-center justify-between gap-2 sm:gap-4`}
              >
                <button
             ...
             px-3.5 sm:px-5 py-2.5 sm:py-3 rounded-2xl
             ...
             transition-all select-none shrink-0"
                ...
                <span className="text-[11px] sm:text-xs font-semibold text-slate-400 dark:text-slate-500 text-center px-1">
                  Question {currentIndex + 1} of {total}
                </span>
                ...
                <div className="shrink-0">
                  {currentIndex === total - 1 ? (
                    <button
                      ...
                      disabled={submitting}
                      className="flex items-center gap-2 bg-green-600 hover:bg-green-700 disabled:opacity-70 disabled:hover:bg-green-600 text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl font-bold text-xs sm:text-sm shadow-md transition-all select-none"
                    >
                      {submitting && (
                    ...
                    <button
                      onClick={() => goTo(currentIndex + 1)}
                      className="flex items-center gap-1.5 bg-brand hover:bg-brand-dark text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-2xl font-bold text-xs sm:text-sm shadow-md transition-all select-none"
                    >
                      Next
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 max-w-sm w-full rounded-3xl p-6 shadow-2xl space-y-4">
            ...
            <div className="flex gap-3 mt-4">
              <button
                ...
                disabled={submitting}
                className="flex-1 flex items-center justify-center gap-2 bg-red-500 hover:bg-red-650 disabled:opacity-70 text-white font-bold py-2.5 rounded-xl text-sm transition"
              >
                {submitting && (
                  <span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin shrink-0" />
                )}
